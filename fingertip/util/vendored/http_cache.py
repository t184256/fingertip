...
def _how_do_I_fetch(log, fetch_sources, url, allow_redirects=False, timeout=2,
                    fallback_to_last=False, warn_on_direct=False):
...
class HTTPCache:
    ...
    def __init__(self, log, host='127.0.0.1', port=0, sources=None,
                 cache_dir=None, fixups_dir=None, warn_on_direct=False):
        if sources is None:
        ...
        self.warn_on_direct = warn_on_direct
        self.host = host
        self._mocks = {}
        ...
        class Handler(RangeHTTPServer.RangeRequestHandler):
            ...
            def _serve(self, uri, headers, meth='GET'):
                ...
                fetch_source, url = _how_do_I_fetch(
                        http_cache.log,
                        http_cache.sources, uri,
                        fallback_to_last=True,
                        warn_on_direct=http_cache.warn_on_direct
                )
                if isinstance(fetch_source, FetchSourceLocal):
                    if meth == 'GET':
                        return super().do_GET()
    ...
    def is_fetcheable(self, url, allow_redirects=False, timeout=2,
        ...
        hdif = _how_do_I_fetch(self.log, sources, url,
                               allow_redirects=allow_redirects,
                               timeout=timeout,
                               warn_on_direct=self.warn_on_direct)
        return hdif != (None, None)
